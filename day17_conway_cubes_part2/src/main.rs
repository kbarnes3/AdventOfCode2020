...
fn do_work<Plane: AsRef<[Row]>, Row: AsRef<[Cube]>>(data: Plane) -> usize {
    ...
    let mut min_coord = Vector4 { x: -1, y: -1, z: -1, w: -1 };
    let mut max_coord = Vector4 { 
    ...
    for _ in 0..CYCLES {
        ...
        for x in min_coord.x..=max_coord.x {
            for y in min_coord.y..=max_coord.y {
                for z in min_coord.z..=max_coord.z {
                    for w in min_coord.w..=max_coord.w {
                        let coordinates = Vector4 { x, y, z, w };
                        ...
                        if let Cube::Active = new_cube {
                            ...
                            min_w = min(min_w, coordinates.w);
                            max_w = max(max_w, coordinates.w);
        ...
        min_coord = Vector4 { x: min_x-1, y: min_y-1, z: min_z-1, w: min_w-1 };
        max_coord = Vector4 { x: max_x+1, y: max_y+1, z: max_z+1, w: max_w+1 };
    }
...
fn count_neighbors(dimension: &HashMap::<Vector4, Cube>, coordinates: &Vector4) -> usize {
    ...
    for x in (coordinates.x-1)..=(coordinates.x+1) {
        for y in (coordinates.y-1)..=(coordinates.y+1) {
            for z in (coordinates.z-1)..=(coordinates.z+1) {
                for w in (coordinates.w-1)..=(coordinates.w+1) {
                    let neighbor = Vector4 { x, y, z, w };
